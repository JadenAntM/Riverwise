    ...
    <main className="shell detail-main">
      ...
      <header className="detail-header">
        <div>
          ...
          <h1>{formatStationName(station.name)}</h1>
      ...
      <section className="candidate-panel" aria-labelledby="candidate-heading">
        <div className="candidate-summary">
          <div>
            <p className="section-kicker" id="candidate-heading">Shadow score candidate</p>
            <p>Seasonal flow, stability, and measured water temperature where available.</p>
          </div>
          <div className="candidate-number">
            {station.candidate_score.value?.toFixed(1) ?? "—"}
            <span>{station.candidate_score.value === null ? "" : "/10"}</span>
          </div>
          <div className="score-status">
            <StatusPill status={station.candidate_score.status} />
            <StatusPill status={station.candidate_score.confidence} />
          </div>
        </div>
        <div className="candidate-reasons">
          <h2>{candidateHeading(station)}</h2>
          <ul>{station.candidate_score.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul>
          {station.candidate_score.available_points > 0 && (
            <p className="coverage">
              {station.candidate_score.earned_points} of {station.candidate_score.available_points} rule points · {station.candidate_score.rules_version}
            </p>
          )}
          <Link href="/score-lab">Compare score versions →</Link>
        </div>
      </section>
      ...
      <section className="detail-grid">
        <article className="info-card">
          ...
          <dl>
            ...
            <div><dt>Nearby air temperature</dt><dd>{formatNumber(station.current_weather?.air_temp_c ?? null, " °C", 1)}</dd></div>
        ...
        <article className="info-card">
          ...
          <dl>
            ...
            <div><dt>14-day median (v1.0)</dt><dd>{formatNumber(station.baseline_median_m3s, " m³/s", 2)}</dd></div>
            <div><dt>Seasonal median (v1.1)</dt><dd>{formatNumber(station.seasonal_median_m3s, " m³/s", 2)}</dd></div>
...
function candidateHeading(station: StationDetail): string {
  if (station.candidate_score.status === "stale") return "No candidate score: the discharge reading is stale.";
  if (station.candidate_score.status === "insufficient_history") return "No candidate score: seasonal history is insufficient.";
  if (station.candidate_score.status === "insufficient_data") return "No candidate score: measured flow is unavailable.";
  return station.candidate_score.confidence === "partial"
    ? "Candidate uses flow inputs; measured water temperature is unavailable."
    : "Candidate uses all three inputs.";
}
